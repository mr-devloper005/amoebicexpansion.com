  ...
  return (
    <EditableSiteShell>
      <main className="mx-auto max-w-6xl px-4 py-10 sm:px-6 lg:px-8">
        <section className="rounded-[2.8rem] bg-[linear-gradient(135deg,#0a395a_0%,#155271_50%,#214d89_100%)] p-6 text-white shadow-[0_30px_90px_rgba(9,60,93,0.18)] sm:p-8">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
            <div>
              <p className="inline-flex items-center gap-2 text-[11px] font-black uppercase tracking-[0.28em] text-[var(--slot4-accent-bright)]">
                <MessageSquare className="h-4 w-4" /> Reader comments
              </p>
              <h1 className="mt-4 font-serif text-4xl font-black tracking-[-0.04em] sm:text-5xl">Comments</h1>
              <p className="mt-4 max-w-2xl text-sm leading-8 text-white/76">Review comments saved in this browser from article pages.</p>
            </div>
            <button type="button" className="rounded-full bg-white px-4 py-2 text-sm font-black uppercase tracking-[0.12em] text-[var(--slot4-page-text)]" onClick={refreshComments}>Refresh comments</button>
          </div>
          ...
          <div className="mt-8 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div className="relative w-full sm:max-w-md">
              ...
              <input
                ...
                className="h-12 w-full rounded-full bg-white pl-11 pr-4 text-sm font-bold text-[var(--slot4-page-text)] outline-none"
              />
            </div>
        ...
        {visibleComments.length ? (
          <section className="mt-8 grid gap-4">
            {visibleComments.map((item) => (
              <article key={`${item.articleSlug}-${item.id}`} className="rounded-[2rem] border border-[var(--slot4-line)] bg-white p-5 shadow-[0_18px_50px_rgba(9,60,93,0.06)]">
          ...
          <section className="mt-8 rounded-[2rem] border border-dashed border-[var(--slot4-line)] bg-white/70 p-8 text-center">
            <h2 className="font-serif text-xl font-black text-[var(--slot4-page-text)]">No comments yet</h2>
        ...
        {filtered.length > COMMENTS_PER_PAGE ? (
          <div className="mt-8 flex flex-wrap items-center justify-between gap-3 rounded-[2rem] border border-[var(--slot4-line)] bg-white p-4 text-sm text-[var(--slot4-muted-text)]">
            <span>Page {currentPage} of {totalPages}</span>
            <div className="flex gap-2">
              <button type="button" className="rounded-full border border-[var(--slot4-line)] px-4 py-2 font-black disabled:opacity-40" disabled={currentPage <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))}>Previous</button>
              <button type="button" className="rounded-full border border-[var(--slot4-line)] px-4 py-2 font-black disabled:opacity-40" disabled={currentPage >= totalPages} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>Next</button>
            </div>
          </div>
        ) : null}
      </main>
