...
const inputClass =
  'h-13 rounded-[1.5rem] border border-[var(--slot4-line)] bg-[#f9fbfb] px-4 text-base font-bold text-[var(--slot4-page-text)] outline-none transition placeholder:text-[var(--slot4-soft-muted-text)] focus:border-[var(--slot4-accent)] focus:bg-white'
const buttonClass =
  'inline-flex h-12 items-center justify-center rounded-full bg-[var(--slot4-page-text)] px-6 text-sm font-black uppercase tracking-[0.16em] text-white shadow-[0_16px_40px_rgba(9,60,93,0.16)] transition hover:-translate-y-0.5 disabled:opacity-60'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`rounded-[1.4rem] px-4 py-3 text-sm font-bold ${status === 'success' ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-700'}`}>{message}</p> : null}
